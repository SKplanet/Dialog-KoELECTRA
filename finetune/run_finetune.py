...
from seq_cls import (
    ...
    PawsProcessor,
    KorSTSProcessor,
    QuestionPairProcessor,
    HateSpeechProcessor,
    seq_cls_load_and_cache_examples,
)
from ner import NaverNerProcessor, ner_load_and_cache_examples
from utils import (
    show_ner_report,
    set_seed,
    compute_metrics,
...
def train(args, model, labels, train_dataset, dev_dataset=None, test_dataset=None):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0 or (
                ...
                if args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    if args.evaluate_test_during_training:
                        result, metric_name = evaluate(
                            args, model, test_dataset, "test", labels, global_step
                        )
                    else:
                        result, metric_name = evaluate(
                            args, model, dev_dataset, "dev", labels, global_step
                        )
                ...
                if (
                    ...
                    if args.save_optimizer:
                        ...
                        logger.info(
                            "Saving optimizer and scheduler states to {}".format(
                                output_dir
                            )
                        )
...
def evaluate(args, model, eval_dataset, mode, labels, global_step=None):
    results = {}
    ...
    if global_step != None:
        logger.info(
            "***** Running evaluation on {} dataset ({} step) *****".format(
                mode, global_step
            )
        )
    else:
    ...
    if task == "naver-ner":
        ...
        resulti, metric_name = compute_metrics(task, out_label_list, preds_list)
    else:
        if args.output_mode == "classification":
            preds = np.argmax(preds, axis=1)
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
        result, metric_name = compute_metrics(task, out_label_ids, preds)
    ...
    if not args.do_nni:
        ...
        output_eval_file = os.path.join(
            output_dir,
            "{}-{}.txt".format(mode, global_step)
            if global_step
            else "{}.txt".format(mode),
        )
