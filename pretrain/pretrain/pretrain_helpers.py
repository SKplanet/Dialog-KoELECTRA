...
def _get_candidates_mask(
    inputs: pretrain_data.Inputs, vocab, tokenizer_type, disallow_from_mask=None
):
...
def mask(
    ...
    candidates_mask = _get_candidates_mask(
        inputs, vocab, config.tokenizer_type, disallow_from_mask
    )
    ...
    if config.tokenizer_type == "sentencepiece":
        inputs_ids, _ = scatter_update(
            inputs.input_ids,
            tf.fill([B, N], vocab.PieceToId("[MASK]")),
